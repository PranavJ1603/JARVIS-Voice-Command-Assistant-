...
if __name__ == "__main__":
    ...
    while True:
        ...
        if "time" in query or "current time" in query:
            ...
            cur.execute(q)
            con.commit()
        elif "google translater" in query or "translater" in query:
            ...
            cur.execute(q)
            con.commit()
        elif "change your name" in query or "change" in query:
            set_name()
            q=f"insert into queries (query)values('{query}');"
            print(q)
            cur.execute(q)
            ...
            cur.execute(q)
            con.commit()
        elif "shutdown" in query or "shut" in query or "down" in query:
        ...
        elif "internet speed" in query or "internet" in query or "speed" in query:
            wb.open("fast.com")
            q=f"insert into queries (query)values('{query}');"
            print(q)
            cur.execute(q)
            con.commit()
        elif "wish me" in query or "wish" in query:
            wishme()

        elif "database" in query:
            speak("Hey! user each and every query will be stored in the mysql workbench whose asked to me if you want to see the each qyuery whose asked to me for this you go to the mysql workbench and the search a database jarvisai and select table from this jarvisai and see whose all data and queries are stored within it ") 
            print("Hey! user each and every query will be stored in the mysql workbench whose asked to me if you want to see the each qyuery whose asked to me for this you go to the mysql workbench and the search a database jarvisai and select table from this jarvisai and see whose all data and queries are stored within it")
